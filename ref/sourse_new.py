...
rough_list = {1250: len(pitch_125), 1500: len(pitch_150), 1750: len(pitch_175), 2000: len(pitch_200)}

depth_index = {0: 0, 1250: len(depth_125), 1500: len(depth_150), 1750: len(depth_175), 2000: len(depth_200)}

rough_list_all = {1250: pitch_125, 1500: pitch_150, 1750: pitch_175, 2000: pitch_200}

depth_index_all = {1250: depth_125, 1500: depth_150, 1750: depth_175, 2000: depth_200}
...
def simu_data_import(filenames):
    data0 = np.loadtxt(f"C:\\Users\\Fujii Kotaro\\project1\\data_all\\cupy_pitch100_depth0.csv")
    data_set_raw = data0
    for filename in filenames:
        data = np.loadtxt(f"C:\\Users\\Fujii Kotaro\\project1\\data_all\\{filename}")
        print(filename)
...
def make_data(data0, data, a1):
    ...
    return extract_frequency_band(freq, fft_data, 2e06, 8e06)[0] / extract_frequency_band(freq, fft_data0, 2e06, 8e06)[
        0
...
def import_data(pitch, depth):
    ...
    for csv_file in csv_files:
        ...
        data_tmp0.append(pd.read_csv(csv_file, encoding="SHIFT-JIS", header=None))
...
def select_pitch_and_image(pitch, depth_list):
    ...
    plt.tight_layout()
    plt.savefig(rf"C:\Users\manat\project2\tmp_output\sim_yfs_{pitch}.eps", format="eps", dpi=300)
    plt.show()
...
def select_pitch_and_image_exp(pitch, depth_list):
    ...
    plt.legend(ncol=2, loc="upper left", fontsize=18)
    plt.tight_layout()
    plt.savefig(rf"C:\Users\manat\project2\tmp_output\exp_yfs_{pitch}.eps", format="eps", dpi=300)
    plt.show()
